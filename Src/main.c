...
Safety_Process();  // Add this line to your existing tasks
   ...
   /* --- Task 9: Temperature Control Processing --- */
TempControl_Process();  // Add this line after Safety_Process()

   /* --- Task 10: Chiller Staging Processing --- */
Staging_Process();
...
else if (strncmp(command, "temp_status", 11) == 0) {
    TempControl_Debug_Status();
}
else if (strncmp(command, "temp_sensors", 12) == 0) {
    TempControl_Debug_Sensors();
}
else if (strncmp(command, "temp_pid", 8) == 0) {
    TempControl_Debug_PID();
}
else if (strncmp(command, "temp_efficiency", 15) == 0) {
    TempControl_Debug_Efficiency();
}
else if (strncmp(command, "temp_setpoint", 13) == 0) {
    // Extract setpoint value from command
    float setpoint = atof(&command[14]);
    TempControl_Debug_SetSetpoint(setpoint);
}
    else {
        char response[100];
        ...
        Send_Debug_Data("- temp_status, temp_sensors, temp_pid, temp_efficiency\r\n");
        Send_Debug_Data("- temp_setpoint <value> : Set temperature setpoint\r\n");
...
/* === TEMPERATURE CONTROL SYSTEM INITIALIZATION === */
Send_Debug_Data("=== Initializing Temperature Control System ===\r\n");
if (!TempControl_Init()) {
    Send_Debug_Data("ERROR: Temperature control system initialization failed!\r\n");
} else {
    Send_Debug_Data("Temperature Control System: READY\r\n");
}

/* --- Task 7: Chiller Staging Processing --- */
Send_Debug_Data("=== Initializing Chiller Staging System ===\r\n");
if (!Staging_Init()) {
    Send_Debug_Data("ERROR: Staging initialization failed\r\n");
} else {
    Send_Debug_Data("Chiller Staging System: READY\r\n");
}
...
/*
 * COMMENTED OUT - Duplicate function definition
 * This function is already implemented in ch_control_core.c
 * 
