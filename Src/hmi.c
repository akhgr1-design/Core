    ...
    if (hmi_index > 0) {
        ...
        if (hmi_index >= 3 && hmi_buffer[0] == 0x5A && hmi_buffer[1] == 0xA5) {
            if (hmi_buffer[2] == 0x06) {
                Send_Debug_Data("*** PERFECT! Complete DWIN response! ***\r\n");
                char success_msg[100];
                snprintf(success_msg, sizeof(success_msg), "*** CRC METHOD '%s' WORKS! ***\r\n", crc_desc);
        ...
        // Test result summary
        char result_msg[150];
        snprintf(result_msg, sizeof(result_msg), "CRC TEST %d/6: %s = %d bytes response\r\n", 
                 (crc_test % 6) + 1, crc_desc, hmi_index);
        Send_Debug_Data(result_msg);
        
    } else {
        Send_Debug_Data("*** NO RESPONSE ***\r\n");
        char fail_msg[100];
        snprintf(fail_msg, sizeof(fail_msg), "CRC METHOD '%s': FAILED\r\n", crc_desc);
        Send_Debug_Data(fail_msg);
    }
    
    Send_Debug_Data("=== END CRC TEST ===\r\n\r\n");
...
void Show_Complete_Message(void) {
    ...
    char info[50];
    snprintf(info, sizeof(info), "Bytes: %d - Data: ", hmi_index);
    Send_Debug_Data(info);

    for (int i = 0; i < hmi_index; i++) {
        char byte_str[5];
        snprintf(byte_str, sizeof(byte_str), "%02X ", hmi_buffer[i]);
        Send_Debug_Data(byte_str);
    }
    Send_Debug_Data("\r\n========================================\r\n\r\n");
